...
function getChrono(){
  ...
  const countdouwnDate = new Date("May 15, 2024").getTime();
  ...
  // % modulo reste de la division (l'opération) précédente divisé par (1000*60*60=1H)
  // console.log(hours);
  const minutes = Math.floor((distanceBase % (1000*60*60)) / (1000*60));
  const seconds = Math.floor((distanceBase % (1000*60)) / 1000);
  // console.log(days, hours, minutes, seconds);

  text.innerText = `${minutes} MIN ${seconds} SEC`;
}
